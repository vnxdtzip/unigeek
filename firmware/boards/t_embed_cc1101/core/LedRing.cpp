...
void LedRing::begin() {
  ...
  FastLED.setBrightness(40);            // cap current draw — 8 px @ full white is a lot
  FastLED.clear(true);
  _began = true;
}
...
void LedRing::update() {
  ...
  if (now - lastMs < 40) return;        // ~25 fps, matches Bruce's LED task cadence
  lastMs = now;
  ...
  if (mode == "rainbow") {
    ...
    if (s_encoderDelta != 0) {          // only advances when the knob turns
