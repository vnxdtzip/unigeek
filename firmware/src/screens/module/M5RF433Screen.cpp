...
void M5RF433Screen::_handleCaptureSelection(uint8_t index) {
  ...
  const M5RF433Util::Signal& sig = _capturedSignals[index];
...
void M5RF433Screen::_sendCapturedSignal(uint8_t index) {
  if (index >= _capturedCount) return;
  if (_txPin < 0) {
    ShowStatusAction::show("TX not available");
    render();
    return;
  }
  ProgressView::init();
...
void M5RF433Screen::_replayStepKeeloqSignal(uint8_t index) {
  if (index >= _capturedCount) return;
  if (_txPin < 0) {
    ShowStatusAction::show("TX not available");
    render();
  ...
  M5RF433Util::Signal& sig = _capturedSignals[index];
