...
## KeeLoq auto-decode
 ...
 > [!danger]
> KeeLoq decode and `Replay +1` are intended for testing remotes, gates, and barriers **you own or have explicit written permission to test**. Running rolling-code bypass against third-party vehicles, garages, or property is illegal in most jurisdictions and prosecuted separately from passive capture (often felony-tier). See [Sub-GHz (CC1101)](sub-ghz.md#replay-with-counter1-rolling-code-bypass) for the scope of what `Replay +1` can and can't bypass.
...
Same format and behavior as the Sub-GHz module — see [Sub-GHz (CC1101)](sub-ghz.md#keeloq-auto-decode) for the keystore file format, the `Mfcodes` menu row, and the limits of counter-step replay.
