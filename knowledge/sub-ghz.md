...
- **Button** — `0`–`15`
- **Counter** — `0x4D7`
- **Fix** — top 32 bits of the reversed payload (always shown, even without a key match)
...
- `mf_name` — free-text label shown in the Info view (e.g. `Aprimatic`, `NICE_Smilo`)
- `hex_key` — 64-bit hexadecimal manufacturer key (with or without `0x` prefix)
- `learning_type` — `1` for simple learning, `2` for normal learning
...
Tap to **reload** the keystore from storage (useful after editing the file via Web File Manager or File Manager). A toast shows the load result.
...
The popup hides `Replay +1` entirely when the signal isn't a decoded KeeLoq — the standard `Replay` is still there for fixed-code remotes.

### .sub fields persisted

Identified KeeLoq captures save the decoded fields alongside the raw key, so reopening the file on any device shows the same breakdown without needing the keystore present:
