...
- **USB Mass Storage** — Expose the SD card to a host as a removable USB drive (card-reader mode) over USB MSC; ESP32-S3 boards with an SD card only. Single-claim USB profile (mutually exclusive with USB MouseKeyboard / WebAuthn per boot); BACK ejects and returns the card to the firmware ([details](knowledge/mass-storage.md))
